...
DIR ="G:/My Drive/KU/Thesis/Thesis/"

survey<-read.csv(file="G:/My Drive/KU/Thesis/data/survey_data/survey_means_weighted.csv",header=TRUE,row.names=1)
# df<-read.csv(file="G:/My Drive/KU/Thesis/Thesis/trained_vectors/trained_vectors/vectors-1800-ngram.txt", header=TRUE,row.names=1, sep=",")
cols<-c(c("Word"), seq(from = 1, to = 300, by = 1))
df<-read.table(file="D:/google_ngrams/Vectors/histwords_engall/1990.txt", header=FALSE,row.names=1, col.names=cols, sep=" ", dec=".")
...
ant_pairs_aff <- read.csv("G:/My Drive/KU/Thesis/data/word_pairs/affluence_antonyms_goc.csv",header=FALSE, stringsAsFactor=F)
ant_pairs_gen <- read.csv("G:/My Drive/KU/Thesis/data/word_pairs/gender_antonyms_goc.csv",header=FALSE, stringsAsFactor=F)
ant_pairs_race <- read.csv("G:/My Drive/KU/Thesis/data/word_pairs/race_antonyms_goc.csv",header=FALSE, stringsAsFactor=F)
...
survey_proj<-na.omit(survey_proj)


cor(survey_proj$affluence_mean,survey_proj$aff_proj)
cor(survey_proj$gender_mean,survey_proj$gender_proj)
cor(survey_proj$race_mean,survey_proj$race_proj)
